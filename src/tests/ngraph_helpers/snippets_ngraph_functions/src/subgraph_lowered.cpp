...
std::shared_ptr<ov::Model> EltwiseFunctionThreeInputsLowered::initLowered() const {
    ...
    auto input_params = makeParams(precision, {input_shapes[0], input_shapes[1], input_shapes[2]});
    auto load_or_broadcastload = [&](size_t i) -> std::shared_ptr<Node> {
        ...
        if (broadcast_shapes[i].empty()) {
            return std::make_shared<snippets::op::Load>(input_params[i]);
        // broadcasting is required: could be Load + BroadcastMove or BroiadcastLoad
            ...
            if (input_shapes[i].back() == 1 && broadcast_shapes[i].back() != 1) {
                return std::make_shared<snippets::op::BroadcastLoad>(input_params[i], broadcast_shapes[i]);
            ...
            // appropriate pointer arithmetics in this case.
                ...
                auto load = std::make_shared<snippets::op::Load>(input_params[i]);
                return std::make_shared<snippets::op::BroadcastMove>(load, broadcast_shapes[i]);
            }
        }
    };
    ...
    auto sub_scalar = std::make_shared<snippets::op::Scalar>(precision, Shape{1}, const_values[0]);
...
//  remove when the code above is enabled:
    ...
    if (broadcast_shapes[2].empty())
        ...
        sub_out = std::make_shared<snippets::op::BroadcastMove>(sub, broadcast_shapes[2]);
    auto mul = std::make_shared<op::v1::Multiply>(add, sub_out);
    auto store = std::make_shared<snippets::op::Store>(mul);
