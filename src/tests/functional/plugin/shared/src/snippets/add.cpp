...
std::string AddConst::getTestCaseName(testing::TestParamInfo<ov::test::snippets::AddConstParams> obj) {
    ...
    std::ostringstream result;
    result << "IS[0]=" << CommonTestUtils::partialShape2str({inputShapes.first}) << "_";
    result << "TS[0]=";
...
std::string AddPair::getTestCaseName(testing::TestParamInfo<ov::test::snippets::AddParamsPair> obj) {
    ...
    std::ostringstream result;
    result << "IS[0]=" << CommonTestUtils::partialShape2str({input_shapes[0].first}) << "_";
    result << "TS[0]=";
    for (const auto& shape : input_shapes[0].second) {
        result << "(" << CommonTestUtils::vec2str(shape) << ")_";
    }
    result << "IS[1]=" << CommonTestUtils::partialShape2str({input_shapes[1].first}) << "_";
    result << "TS[1]=";
