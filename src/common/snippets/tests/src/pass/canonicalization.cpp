...
TEST_F(CanonicalizationTests, smoke_Snippets_Canonicalization_1) {
    ...
    prepare_functions(m_input_shapes);
    normalizeParameter(function_ref->get_parameters()[1], 2, 0);
    run();
}
...
TEST_F(CanonicalizationTests, smoke_Snippets_Canonicalization_2) {
    ...
    const auto& params = function_ref->get_parameters();
    ...
    function->get_parameters()[0]->set_partial_shape(PartialShape(m_input_shapes[0]));
    ...
    function_ref->validate_nodes_and_infer_types();
    run();
}
