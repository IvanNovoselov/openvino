...
Subgraph::VectorDims Subgraph::get_master_shape() {
    ...
    if (m_is_dynamic) {
        ...
        for (const auto& res : body_ptr()->get_results()) {
            ...
            const auto& planar_shape = utils::get_port_planar_shape(res_input.get_source_output());
    ...
    const auto& default_broadcasting = std::make_shared<entryNumpyBroadcasting>();
...
void Subgraph::control_flow_transformations(lowered::LinearIR& linear_ir,
                                            ...
                                            const lowered::pass::PassPipeline& post_common) const {
    INTERNAL_OP_SCOPE(Subgraph);
...
snippets::Schedule Subgraph::generate(const BlockedShapeVector& blocked_input_shapes,
    ...
    return generate_from_linear_ir(pre_common, post_common, compile_params);
}

snippets::Schedule Subgraph::generate_from_linear_ir(const lowered::pass::PassPipeline& pre_common,
                                                     const lowered::pass::PassPipeline& post_common,
    ...
    LoweringResult lowering_result;
    control_flow_transformations(linear_ir, lowering_result, pre_common, post_common);
    ...
    return {work_domain, std::move(lowering_result)};
}
