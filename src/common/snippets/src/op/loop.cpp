...
LoopBase::LoopBase(const std::vector<Output<Node>> &args, size_t dimension, size_t work_amount, size_t wa_increment)
        : Op(args), dimension(dimension), work_amount(work_amount), wa_increment(wa_increment), evaluate_once(false) {
}
...
LoopBegin::LoopBegin(const std::vector<Output<Node>> &args, size_t dimension, size_t work_amount, size_t wa_increment)
        : LoopBase(args, dimension, work_amount, wa_increment),
...
std::shared_ptr<Node> LoopBegin::clone_with_new_inputs(const OutputVector& inputs) const {
    auto res = std::shared_ptr<LoopBegin>(new LoopBegin(inputs, dimension, work_amount, wa_increment));
    res->avoid_scalar_loop_injection = avoid_scalar_loop_injection;
    return res;
//    return std::shared_ptr<LoopBegin>(new LoopBegin(inputs, dimension, work_amount, wa_increment));
}
...
LoopEnd::LoopEnd(const std::vector<Output<Node>> &args, size_t dimension, size_t work_amount, size_t wa_increment,
                 std::vector<bool> apply_increments, std::vector<int64_t> finalization_offsets)
        : LoopBase(args, dimension, work_amount, wa_increment), finalization_offsets(std::move(finalization_offsets)),
        has_outer_loop(true) {
...
LoopEnd::LoopEnd(const std::vector<Output<Node>> &args, size_t dimension, size_t work_amount, size_t wa_increment,
        ...
        : LoopBase(args, dimension, work_amount, wa_increment), ptr_increments(std::move(ptr_increments)),
...
std::shared_ptr<Node> LoopEnd::clone_with_new_inputs(const OutputVector& inputs) const {
    return std::make_shared<LoopEnd>(inputs, dimension, work_amount, wa_increment, ptr_increments, finalization_offsets);
}
