...
static void one_hot_bench_4(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        // Set input-depending variables
        state.PauseTiming();
        const size_t in_ax_size(state.range(0));
        std::vector <int64_t> indices(in_ax_size * in_ax_size);
        std::generate(indices.begin(), indices.end(), [&gen, &dist]() { return dist(gen); });
        const auto indices_const = op::Constant::create(element::i64, Shape{in_ax_size, in_ax_size}, indices);
        auto ind_p = std::make_shared<HostTensor>(indices_const);
        HostTensorVector input = {ind_p, depth_p, on_p, off_p};
        auto oup = std::make_shared<HostTensor>(std::make_shared<op::v0::Constant>(element::i64, Shape{in_ax_size, in_ax_size, 4}));
...
//BENCHMARK(one_hot_bench_4)->Unit(benchmark::kMicrosecond)->RangeMultiplier(2)->Range(4, 2<<8)->Complexity();
BENCHMARK(one_hot_bench_4)->RangeMultiplier(2)->Range(4, 2<<8)->Complexity();
...
static void one_hot_bench_256(benchmark::State& state)
{
    std::vector <int64_t> indices(256);
    std::random_device rnd;
    std::mt19937 gen(1);
    std::uniform_int_distribution<> dist(0.,10.);
    std::generate(indices.begin(), indices.end(), [&gen, &dist]() { return dist(gen); });
    ...
    const auto indices_const = op::Constant::create(element::i64, Shape{16, 16}, indices);
    const auto depth_const = op::Constant::create(element::i64, Shape{}, {4});
    const auto on_const = op::Constant::create(element::i64, Shape{}, {on_value});
    const auto off_const = op::Constant::create(element::i64, Shape{}, {off_value});
    int64_t axis = -1;
    ...
    //std::vector <int64_t> out_v(48);
    auto oup = std::make_shared<HostTensor>(std::make_shared<op::v0::Constant>(element::i64, Shape{16, 16, 4}));
    HostTensorVector output = {oup};
...
BENCHMARK(one_hot_bench_256)->Unit(benchmark::kMicrosecond);
*/